...
closes the actual socket. Confirmed live: this exhausted Render's
Postgres connection limit, after which new connections hung forever
waiting for a slot. First fix: every method explicitly closed its own
connection in a `finally` block.

CONNECTION LEAK FIX, PART 2 (2026-08-18): Part 1 was correct but still
left every method opening and closing its OWN fresh connection --
app/lakehouse_client.py's get_zone_stats() alone calls table_exists()/
load_table() up to ~7 times per HTTP request, so one request still meant
up to 7 sequential connect/disconnect round trips. Confirmed live: after
Part 1 shipped, simpler endpoints (one connection each) started
succeeding again (slowly, while the pool was still draining leftover
pre-fix connections) while this catalog's own callers kept failing --
they needed several free slots at once, not just one. This catalog
instance now opens ONE connection lazily on first use (_get_conn()) and
reuses it for every method call made on that SAME instance -- callers
MUST call close() (or use the catalog as a context manager: `with
_iceberg_catalog() as catalog:`) when done with it, since the connection
is no longer closed automatically after each individual method call.
"""
...
class PostgresIcebergCatalog(MetastoreCatalog):
    ...
    def close(self) -> None:
        ...
        of its methods (e.g. lakehouse_client.get_zone_stats(), or a DAG
        task using it across several _write_iceberg_table() calls) MUST
        call this when done with the instance -- or use it as a context
        manager (`with _iceberg_catalog() as catalog:`), which calls this
        automatically. The connection is no longer closed automatically
        after each individual method call (see module docstring)."""
