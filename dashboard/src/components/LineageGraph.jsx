...
// Shared lineage graph -- fetch + layout + render, extracted 2026-08-20
// from what was originally FieldLineage.jsx's own inline logic. Used by
// two call sites now: the standalone Field Lineage page
// (pages/FieldLineage.jsx, full-size) and embedded directly on the
// Data Catalog page (pages/DataCatalog.jsx, compact -- closes the gap
// where the graph only existed as a link to a separate tab and was
// never actually rendered on the Catalog page itself). One fetch, one
// layout implementation, shared -- a fix to either shows up in both
// places automatically instead of drifting apart.
//
// See app/marquez_client.py's module docstring for the full story on
// how this graph is actually built: Marquez's own /datasets endpoint
// was confirmed empty in this setup, so get_field_lineage() assembles
// the graph itself from each job's latest run facet's real
// inlets/outlets -- genuine data, just not routed through Marquez's
// own dataset registry. Every node/edge traces to an actual S3 path a
// real task actually read from or wrote to.
...
// `height` sizes the ReactFlow canvas -- the standalone Field Lineage
// page passes 420 (unchanged from before extraction), DataCatalog
// passes a smaller value for its embedded slot. `showEmptyState` /
// `showFootnote` let an embedding page suppress the "no lineage yet"
// card and the explanatory footnote when it wants its own wording
// instead (DataCatalog keeps its messaging terser to match its page).
export function LineageGraph({ height = 420, showEmptyState = true, showFootnote = true }) {
  ...
  return (
    <div>
      ...
      {state.data && !state.data.configured && (
        <div className="mb-3 text-[12.5px] text-ink-soft bg-[#FFF8E8] border border-[#F0DFAE] rounded-xl px-4 py-3">
          Not connected yet — set <code className="font-mono">MARQUEZ_URL</code> on this service.
        </div>
      )}

      {showEmptyState && state.data?.configured && !hasGraph && !state.loading && !state.error && (
      ...
      {hasGraph && showFootnote && (
        <div className="mt-3 text-[11.5px] text-ink-faint leading-relaxed max-w-2xl">
